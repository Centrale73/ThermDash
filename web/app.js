...
export const keys = {
  get: provider => localStorage.getItem(`td.key.${provider}`) || '',
  set: (provider, val) => {
    if (val && val.trim()) localStorage.setItem(`td.key.${provider}`, val.trim());
    else localStorage.removeItem(`td.key.${provider}`);
  },
  has: provider => Boolean(localStorage.getItem(`td.key.${provider}`)),
  all: () => ({
    perplexity: localStorage.getItem('td.key.perplexity') || '',
    anthropic: localStorage.getItem('td.key.anthropic') || '',
    openai: localStorage.getItem('td.key.openai') || '',
  }),
};
...
const ROUTES = {
  ...
  '/environmental': () => import('/web/routes/environmental.js'),
  '/chat':          () => import('/web/routes/chat.js'),
